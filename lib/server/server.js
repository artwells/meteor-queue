...
Queue.setInterval = function(name, command, interval){
    ...
    entryarray.handle = Meteor.setInterval(function(){eval(command)}, interval);

    try {
        res = Queue.intervals.insert(entryarray);
        if (Queue.loglevel > 2) {
                Queue.log.insert({command: 'Queue.interval added: ' + entryarray.name, status: 'success', data: 'handle:' + entryarray.handle, created_at: new Date()});
        }
    } catch (e) {
        if (Queue.loglevel > 0) {
            /* otherwise include the whole stack */
...
/*
Queue.clearInterval = function(handle) {
    Meteor.clearInterval(handle);
    Queue.intervals = Queue.intervals.filter(function (el) {
    ...
    Meteor.startup(function () {
        Meteor.methods({
            addQueueRunNow : function() {   
                if (typeof '_houston_queue/Run Now' === "undefined") {
            ...
            getQueueIntervalLength : function() {
                return Queue.intervals.length;
            },
            getQueueIntervals : function() {
                var t = Queue.intervals[0];
                return EJSON.stringify(t);
             //   return [{test: true},{mud:"hup"}];
         }
