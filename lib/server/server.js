...
Queue.setInterval = function(name, command, interval){
    ...
    try {
        res = Queue.queueintervals.insert(entryarray);
        Queue.intervalhandles[res] = handle;
        if (Queue.loglevel > 2) {
...
//                             Queue.queueintervalhandles[queueinterval._id]
        ...
        /* sometimes get
        * Exception while invoking method 'addQueueRunNow' Error: A method named '_houston_queue/Run Now' is already defined */
        console.log(e);
    }
    });
}
